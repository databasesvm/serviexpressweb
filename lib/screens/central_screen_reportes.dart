...
// ══════════════════════════════════════════════════════════════════════════════
// PANEL DE REPORTES — pantalla completa (Navigator.push)
// ══════════════════════════════════════════════════════════════════════════════

class _PanelReportesScreen extends StatefulWidget {
  const _PanelReportesScreen();

  @override
  State<_PanelReportesScreen> createState() => _PanelReportesScreenState();
}

class _PanelReportesScreenState extends State<_PanelReportesScreen>
    with SingleTickerProviderStateMixin {
  late final TabController _tab;
  bool _cargando = true;
  List<Map<String, dynamic>> _reportes = [];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: TabBarView(
        controller: _tab,
        children: [_tabQuejas(), _tabHorasActivas()],
      ),
    );
  }
    ...
    return Column(children: [
      ...
      Expanded(
        child: _cargando
            ? const Center(child: CircularProgressIndicator(color: Colors.orange))
            : _reportesFiltrados.isEmpty
                ? const Center(child: Text('Sin reportes', style: TextStyle(color: Colors.white38)))
                : ListView.separated(
                    padding: const EdgeInsets.all(12),
                    itemCount: _reportesFiltrados.length,
                        ...
                        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          ...
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            Row(children: [
                              ...
                              const SizedBox(width: 6),
                              Text('Servicio #${r['servicio_id']}',
                                  style: const TextStyle(color: Colors.white70, fontSize: 11)),
                              const Spacer(),
                              Text(_formatFecha(r['created_at']?.toString()),
                                  style: const TextStyle(color: Colors.white30, fontSize: 10)),
                            ]),
                            const SizedBox(height: 4),
  ...
  Widget _tabHorasActivas() {
    return _cargandoHoras
        ? const Center(child: CircularProgressIndicator(color: Colors.orange))
        : _horasMoviles.isEmpty
            ? const Center(child: Text('Sin datos de hoy', style: TextStyle(color: Colors.white38)))
            : Column(children: [
                Padding(
                  padding: const EdgeInsets.fromLTRB(12, 8, 12, 4),
                  child: Row(children: [
                    const Text('Acumulado de sesiones cerradas hoy',
                        style: TextStyle(color: Colors.white38, fontSize: 10)),
                    const Spacer(),
                    IconButton(
                      icon: const Icon(Icons.refresh, color: Colors.white38, size: 16),
                    ...
                    return Padding(
                      ...
                      child: Row(children: [
                        Container(width: 8, height: 8, margin: const EdgeInsets.only(right: 10),
                            decoration: BoxDecoration(
                              color: enLinea ? Colors.green : Colors.white24, shape: BoxShape.circle)),
                        Text((m['usuario'] ?? '').toString().toUpperCase(),
                            style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13)),
                        if (enLinea) ...[
                          const SizedBox(width: 6),
                          const Text('EN LÍNEA',
                        ...
                        const Spacer(),
                        Text(_formatMinutos(mins),
                            style: TextStyle(color: mins > 0 ? Colors.orange[300] : Colors.white30,
