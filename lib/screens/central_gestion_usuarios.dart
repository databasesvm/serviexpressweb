...
class _PanelGestionUsuariosState extends State<_PanelGestionUsuarios>
  ...
  void initState() {
    super.initState();
    _tabCtrl = TabController(length: 8, vsync: this, initialIndex: widget.tabInicial);
    _cargar();
  }
    ...
    try {
      ...
      final results = await Future.wait([
        ...
        _db.from('usuarios')
            ...
            .order('created_at'),
        _db.from('usuarios')
            .select('id, nombre, usuario, rango_movil, puntuacion, activo, tipo_plan_movil, numero_movil, saldo_wallet, comision_pct, wallet_bloqueado, tiene_fn, tiene_se, es_dual')
  ...
  // ── Toggle acceso dual (central + móvil) ────────────────────────────────
  Future<void> _toggleDualDialog(Map<String, dynamic> u) async {
    final nombre = movilLabel(u);
    final esDual = u['es_dual'] == true;
    final accion = esDual ? 'quitar' : 'activar';
    final confirmado = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(children: [
          Icon(
            esDual ? Icons.person_off_rounded : Icons.supervisor_account_rounded,
            color: esDual ? Colors.orange : const Color(0xFF818CF8),
            size: 18,
          ),
          const SizedBox(width: 8),
          Expanded(child: Text(
            esDual ? 'Quitar acceso de central' : 'Activar modo dual',
        ...
        content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(
            esDual
                ? 'Al $accion el modo dual, $nombre dejará de ver el panel de central al iniciar sesión y solo podrá operar como móvil.'
                : 'Al $accion el modo dual, $nombre verá un selector al iniciar sesión para elegir entre operar como móvil o acceder al panel de central.',
        ...
        actions: [
          ...
          ElevatedButton(
            style: ElevatedButton.styleFrom(
              backgroundColor: esDual ? Colors.orange[700] : const Color(0xFF818CF8),
              foregroundColor: Colors.white,
            ),
            onPressed: () => Navigator.pop(ctx, true),
            child: Text(esDual ? 'QUITAR DUAL' : 'ACTIVAR DUAL',
    ...
    try {
      await _db.from('usuarios').update({'es_dual': !esDual}).eq('id', u['id']);
      final idx = _moviles.indexWhere((m) => m['id'] == u['id']);
      if (idx != -1) setState(() => _moviles[idx] = {..._moviles[idx], 'es_dual': !esDual});
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(!esDual
            ? '✅ $nombre ahora tiene acceso dual (central + móvil)'
            : '✅ $nombre ya solo opera como móvil'),
        backgroundColor: !esDual ? const Color(0xFF818CF8) : Colors.orange[700],
      ));
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(
                ...
                child: SingleChildScrollView(
                  ...
                  child: Row(
                    children: [
                      ...
                      const SizedBox(width: 8),
                      _statBox('${_locales.length}', 'Locales', const Color(0xFFF59E0B), onTap: () => _tabCtrl.animateTo(6)),
                      const SizedBox(width: 8),
                      _statBox('${_clientes.length}', 'Clientes', const Color(0xFF22C55E), onTap: () => _tabCtrl.animateTo(7)),
                      const SizedBox(width: 8),
          ...
          child: Padding(
            ...
            child: LayoutBuilder(builder: (context, constraints) {
              ...
              final botones = Row(mainAxisSize: MainAxisSize.min, children: [
                ...
                _botonAccion(
                  esDual ? Icons.supervisor_account_rounded : Icons.manage_accounts_rounded,
                  esDual ? const Color(0xFF818CF8) : Colors.white38,
                  'Dual',
                  () => _toggleDualDialog(u),
                ),
  ...
  // ── TAB WALLET fue extraído a _PanelBilletera (central_billetera.dart) ──────
